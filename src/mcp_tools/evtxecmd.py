...
mcp_tools.evtxecmd
==================
Registers the run_evtxecmd MCP tool.
...
def register(mcp: FastMCP, tool_cfg: dict, output_dir: Path) -> None:
    ...
    @mcp.tool(name="run_evtxecmd", description=tool_cfg["description"])
    async def run_evtxecmd(
        path: str,
        ...
        """
        Run EVTXECmd against a Windows Event Log (.evtx) file or directory.
        ...
        """
        out_dir = output_dir / "evtxecmd" / safe_stem(path)
        ...
        job_id = registry.submit("run_evtxecmd", _work())

        return JobSubmitted(
            job_id=job_id,
            tool_name="run_evtxecmd",
            status="pending",
            message=f"EVTXECmd started. Poll get_job_status('{job_id}') to check progress.",
        )
